...
- **型**: bool
- **デフォルト**: `false`
- **説明**: `true` のとき、`buildtime_packages`（`@genpack-buildtime` セット）を、メインの emerge（`@world @genpack-runtime @genpack-buildtime`）とは**別の先行パスで先に** emerge します。通常 `buildtime_packages` は「Lower に入れて最終イメージから除外する」という*配置*の意味しか持たず、順序は portage が宣言依存から解決します。しかし、ある runtime パッケージが**宣言依存に現れない**ビルド環境ツールを必要とする場合（例: `package.env` で `gentoo-kernel` に clang/LLVM ツールチェーンを差し替える）、依存エッジが無いため portage は順序を保証できず、`--parallel` 時はレースになります。このフラグはそうしたアーティファクトが「ツールチェーンをビルド環境へ先に据える」ことをオプトインするためのものです（別 emerge プロセスなのでメインパスでは PATH/`profile.env` も更新済みになります）。詳細は ADR-0003 を参照。

```json5
{
  buildtime_packages: ["llvm-core/clang", "llvm-core/lld", "llvm-core/llvm"],
  buildtime_packages_first: true
}
```
...
マージ可能なフィールド: `packages`, `buildtime_packages`, `buildtime_packages_first`, `accept_keywords`, `use`, `mask`, `license`, `env`, `binpkg_excludes`, `setup_commands`, `services`
