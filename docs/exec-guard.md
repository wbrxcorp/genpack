...
これ以降のすべての `execve()` と `mmap(PROT_EXEC)` に対し、以下のルールが適用されます。
...
exec-guard を有効にしたイメージには `/etc/audit/rules.d/exec_guard.rules` が自動的にインストールされます。auditd が起動するとこのルールが読み込まれ、ブロックされた `execve()` および `mmap()` が `key=exec_guard_deny` でログに記録されます。
...
| `mmap(PROT_READ)` → `mprotect(+EXEC)` の 2 ステップ | `mmap_file` フックで捕捉不可 |
| カーネル脆弱性によるページキャッシュ改ざん | カーネル自体の信頼が前提 |
| anonymous `mmap(PROT_EXEC)` へのシェルコード書き込み・実行 | ファイルを持たない匿名マッピングは `mmap_file` フックの対象外。塞ぐことは技術的には可能だが、Python・Java・Node.js 等の JIT コンパイラが同じ仕組みを正当な用途で使っており、どのパッケージが JIT を使うか実行前に把握することが難しいため、現実的ではない |
