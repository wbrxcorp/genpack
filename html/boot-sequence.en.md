...
A combination of `boot.img` (MBR stage 1) and `core.img` (generated by `grub-mkimage`). The prefix `(,msdos1)/boot/grub` is hardcoded into `core.img`, which reads `/boot/grub/grub.cfg` from the boot partition. For BIOS, grub.cfg is not embedded in the binary — `genpack-install` places it on the boot partition during disk installation.
...
Attempts COM0 at 115200 baud. On success, both serial and console are configured as input/output terminals.

**2. Boot partition identification**

Determines the boot partition from the GRUB variable `$cmdpath` (the path from which the bootloader was loaded) and obtains its UUID via `probe -u`.

**3. System image detection**

Searches for the SquashFS image in the following order:

1. `system.img` on the boot partition
2. `system` on the data partition (searched by label `data-<UUID>` → `d-<UUID>` → fallback to partition number)

**4. SquashFS mount and kernel detection**

Loop-mounts the SquashFS via the `loopback` command and switches the root with `set root=loop`. If the image contains `/boot/grub/grub.cfg`, GRUB delegates to it via `configfile` (allowing the image to override the boot configuration).

If no such file exists, processing continues as follows.

**5. Timeout determination**

If `boottime.txt` persists on the boot partition (evidence of a previous unclean shutdown [^1]), the timeout is set to 10 seconds; otherwise it is set to 1 second.

**6. Custom configuration loading**

If `system.cfg` exists on the boot partition, it is loaded via `source`. This file can set the `LINUX_ARGS` variable to customize the kernel command line.

**7. Kernel command line construction**

- `panic=30` is added by default (unless explicitly specified)
- On x86, `console=ttyS0,115200n8r console=tty0` is added (unless explicitly specified)

**8. Menu entries**

| Entry | Kernel command line |
|---|---|
...
The kernel and initramfs are loaded from `/boot/kernel` and `/boot/initramfs` inside the SquashFS (since the image is already loop-mounted, GRUB can reference files within it directly). If MemTest86 is available, additional menu entries are shown.
