...
Loop-mounts the SquashFS via the `loopback` command and switches the root with `set root=loop`. If the image contains `/boot/grub/grub.cfg`, GRUB delegates to it via `configfile` [^7].

If no such file exists, processing continues as follows.

**5. Timeout determination**

If `boottime.txt` persists on the boot partition (evidence of a previous unclean shutdown [^1]), the timeout is set to 10 seconds [^6]; otherwise it is set to 1 second.

**6. Custom configuration loading**

If `system.cfg` exists on the boot partition, it is loaded via `source` [^8]. This file can set the `LINUX_ARGS` variable to customize the kernel command line.
